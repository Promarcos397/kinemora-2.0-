...
const HomePage: React.FC<PageProps> = ({ onSelectMovie, onPlay, seekTime }) => {
  const { myList, continueWatching, getLikedMovies, isKidsMode } = useGlobalContext();
  ...
  const rows = useMemo<SmartRow[]>(() => {
    ...
    if (isKidsMode) {
      manifest.push({ key: 'kids-continue', title: 'Start Watching', fetchUrl: REQUESTS.fetchKidsHero });
      manifest.push({ key: 'kids-play', title: 'Fairies, Fiends, Film Time!', fetchUrl: REQUESTS.fetchKidsTVPlayful });
      manifest.push({ key: 'kids-family', title: 'Calm and Gentle', fetchUrl: REQUESTS.fetchKidsTVFamily });
      manifest.push({ key: 'kids-moves', title: 'Adventures Galore', fetchUrl: REQUESTS.fetchKidsMoviesAnimation });
      manifest.push({ key: 'kids-family-movies', title: 'Family Movie Night', fetchUrl: REQUESTS.fetchKidsMoviesFamily });
      manifest.push({ key: 'kids-trending', title: 'Trending Now', fetchUrl: REQUESTS.fetchKidsTVTrending });
      return manifest;
    }
    ...
    if (continueWatching.length > 0) {
      ...
      manifest.push({
        ...
        title: `More like ${seed.title || seed.name}`,
    ...
    if (myList.length > 0) {
      ...
      manifest.push({
        ...
        title: `Because you added ${seed.title || seed.name} to your List`,
        fetchUrl: REQUESTS.fetchRecommendations(getMediaType(seed), seed.id),
      });
    }
    ...
    manifest.push({
      key: 'award-winning',
      title: 'Award-winning Bingeworthy Series',
    ...
    if (continueWatching.length > 2) {
      ...
      manifest.push({
        ...
        title: `Because you watched ${seed.title || seed.name}`,
        fetchUrl: REQUESTS.fetchRecommendations(getMediaType(seed), seed.id),
      });
    }
    ...
    if (likedEntries.length > 0) {
      ...
      manifest.push({
        ...
        title: `Because you liked ${seed.title || seed.name}`,
        fetchUrl: REQUESTS.fetchSimilar(getMediaType(seed), seed.id),
      });
    }
    ...
    manifest.push({
      ...
      title: 'Familiar Favourite Series',
    ...
    // --- ROW 21: Imaginative Series ---
    manifest.push({
      key: 'imaginative',
      title: 'Imaginative Series',
      fetchUrl: REQUESTS.fetchImaginativeSeries,
    });

    // --- ROW 22: Because you liked [2nd liked] ---
  ...
  return (
    <>
      <HeroCarousel
        ...
        onPlay={onPlay}
        fetchUrl={isKidsMode ? REQUESTS.fetchKidsHero : REQUESTS.fetchPopular}
        seekTime={seekTime}
      />
