...
import { fetchTrailer, getMovieCredits, getMovieDetails, getRecommendations, getSeasonDetails } from '../services/api';
...
const InfoModal: React.FC<InfoModalProps> = ({ movie, onClose, onPlay }) => {
  ...
  useEffect(() => {
    if (movie) {
      ...
      // 2. Fetch Trailer
      fetchTrailer(movie.id, mediaType).then(key => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-x-8 gap-y-6">
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-white font-medium text-sm md:text-base mt-2">
                        <span className="text-[#46d369] font-bold">{(activeMovie.vote_average * 10).toFixed(0)}% Match</span>
