...
const VideoPlayerControls: React.FC<VideoPlayerControlsProps> = ({
    ...
    return (
        <div className="absolute inset-x-0 bottom-0 z-50 bg-gradient-to-t from-black via-black/80 to-transparent px-8 pb-8 pt-20 pointer-events-auto transition-opacity duration-300">
            {/* Timeline - Hidden if Menu is Open */}
            <div
                ref={timelineRef}
                className={`relative w-full h-1.5 bg-gray-600/50 hover:h-2.5 transition-all duration-200 rounded-none mb-6 group/timeline cursor-pointer flex items-center ${isMenuOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
                ...
                <div className="flex items-center space-x-6 z-10">
                    {onNextEpisode && showNextEp && (
                        <button onClick={(e) => { e.stopPropagation(); onNextEpisode(); }} className="text-white text-lg uppercase tracking-wide flex items-center transition hover:opacity-80">
                            <span className="mr-1"><SkipForwardIcon size={32} weight="bold" /></span> Next Ep
                        </button>
                    )}
