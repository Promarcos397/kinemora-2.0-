    ...
    <div className="group relative my-4 md:my-6 space-y-2 z-10 hover:z-50 transition-all duration-300">
      <h2 className="px-6 md:px-14 lg:px-20 text-sm sm:text-base md:text-lg font-bold text-[#e5e5e5] hover:text-white transition cursor-pointer flex items-center group/title w-fit">
        ...
        <span className="text-xs text-cyan-500 ml-2 opacity-0 group-hover/title:opacity-100 transition-opacity duration-300 flex items-center font-semibold">
          Explore All <span className="material-icons text-sm ml-1">chevron_right</span>
        </span>
      </h2>
      ...
      <div className="relative group/row">
        ...
        <div 
          ...
          onClick={() => scroll('left')}
        >
          <span className="material-icons text-white text-3xl md:text-5xl hover:scale-125 transition drop-shadow-lg">chevron_left</span>
        </div>
          ...
          {loading 
            ? Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="relative flex-none h-[120px] w-[140px] sm:h-[150px] sm:w-[180px] md:h-[180px] md:w-[210px] lg:h-[210px] lg:w-[240px] mr-4 flex items-end">
                     ...
                     {/* Image Skeleton */}
                     <div className="h-full w-[70%] bg-[#222] rounded-md border border-white/5 animate-pulse relative overflow-hidden">
            ...
            >
              ...
              <img 
                ...
                className="h-full w-auto object-cover rounded-md z-10 shadow-lg ml-auto" 
        ...
        >
          <span className="material-icons text-white text-3xl md:text-5xl hover:scale-125 transition drop-shadow-lg">chevron_right</span>
        </div>
      </div>
    </div>
  );
