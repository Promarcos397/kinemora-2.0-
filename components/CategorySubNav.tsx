        ...
        <div className="relative z-30 flex items-center justify-between px-6 md:px-14 lg:px-16 pt-2 pb-3">
            ...
            <div className="hidden md:flex items-center gap-1">
                <button
                    ...
                    className={`p-1.5 rounded-sm transition-colors ${viewMode === 'row' ? 'text-white' : 'text-white/40 hover:text-white/70'}`}
                    title="Row View"
                >
                    <ListBulletsIcon size={22} weight={viewMode === 'row' ? 'bold' : 'regular'} />
                </button>
                <button
                    onClick={() => onViewModeChange?.('grid')}
                    className={`p-1.5 rounded-sm transition-colors ${viewMode === 'grid' ? 'text-white' : 'text-white/40 hover:text-white/70'}`}
                    title="Grid View"
                >
                    <SquaresFourIcon size={22} weight={viewMode === 'grid' ? 'fill' : 'regular'} />
                </button>
            </div>
        </div>
    );
